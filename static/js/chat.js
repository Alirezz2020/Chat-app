...
document.addEventListener("DOMContentLoaded", function() {
  ...
  document.getElementById("chat-message-send").addEventListener("click", function() {
    ...
    if (fileInput.files.length > 0) {
      // Upload the file via AJAX first
      ...
      .then(data => {
        if (data.success) {
          ...
          // Now send the WebSocket message with both text and attachment URL
  ...
  // Optionally add event listeners for edit/delete buttons here.
});
if (fileInput.files.length > 0) {
  const file = fileInput.files[0];
